...
function CommitGraph() {
  ...
  return (
      <section>
        <div className="flex flex-col items-center justify-center w-52 p-6">
              ...
              >
                {week.map((commitCount, j) => (
                  <div
                    key={`week-${i}-day-${j}`}
                    id={`week-${i}-day-${j}`}
                    className={cn('h-1 w-1 sm:h-2 sm:w-2 md:h-3 md:w-3 lg:h-4 lg:w-4  md:rounded-[2px]',getColor(commitCount))}
                  />
                ))}
              </div>
            ))}
