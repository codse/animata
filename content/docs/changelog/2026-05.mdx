...
<ChangeLogComponents>
  ...
  <ChangeLogEntry href="/docs/card/card-spread">
    <h3>Card Spread</h3>
    <p>Reworked the deck, hover peek, and click-to-spread motion with transform-only layout, split inner/outer layers, and a quick throw ease on expand.</p>
  </ChangeLogEntry>
  <ChangeLogEntry href="/docs/preloader/split-reveal">
    <h3>Split Reveal</h3>
...
Migrated from Yarn 4 to pnpm. Upgraded Tailwind CSS to 4.3 and re-synced seventeen shadcn/ui primitives from `https://animata.design/r/{category}/{name}.json`. The `shadcn:add` script calls `shadcn@latest`. Docs use Instrument Sans and silver inline code chips; fenced blocks get line numbers after pretty-code. Animated Border Trail install steps no longer reference unused global CSS. Unpublished feature-cards docs and Commit Graph. Unpublished Calendar Widget, Music Stack Interaction, Team Clock, and Video Chat from the docs and shadcn registry. Refined the Wave 1 widget set — Reminder, Weather Card, Water Tracker, VPN, Delivery Card, Alarm Clock, Calendar Event, and Music Widget — with Apple-style type hierarchy, glanceable props, named Storybook scenarios, and docs aligned to the widget overview. Replaced bare `transition` utilities with explicit `transition-*` across animata.
