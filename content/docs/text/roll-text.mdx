...
Site header nav uses the same pattern: `data-roll-group` on the `Link`, `groupHover` + `pointer-events-none` on `RollText`, fixed color on `RollText` (not `hover:text-*` on the link), and `disabled` on the active item.
...
<PropsTable>
  ...
  <PropsTable.Row prop="disabled" type="boolean" default="false">
    Skip the roll on hover and focus — use for active nav items or other static states.
  </PropsTable.Row>
